...
const VoteForm1 = ({close1})=>{
    const {addAdminByOwner} = useContext(ContextApi);
    const [address, setCreatorName] = useState(``);
    console.log(address, 'sdd')
    const createVote = ()=>{
        addAdminByOwner(address);
    }

    return(
        <>
           <div className="w-[350px] px-4 py-10 rounded-xl relative bg-white">
             <button onClick={close1} className="top-1 right-1 absolute"><img className="w-[30px] h-[30px]" src="https://i.ibb.co/1mswmRF/close.png" alt="sea evolution"/></button>
            <h2 className="text-center w-full mb-6 font-black text-xl ">Add another Admin</h2>
